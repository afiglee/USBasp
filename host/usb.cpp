...
int USB::usbOpenDevice(int vid, const string &usbvendor, int pid,
  ...
  for (j = 0; j < dev_list_len; ++j) {
    ...
    debug << "Found vid=0x" << std::hex << descriptor.idVendor << " pid=0x"
          << descriptor.idProduct << std::dec << std::endl;
    if (descriptor.idVendor == vid && descriptor.idProduct == pid) {
      ...
      if (!handle) {
        ...
        warning << "cannot open USB device: vid=0x" << std::hex
                << descriptor.idVendor << " pid=0x" << descriptor.idProduct
                << std::dec << std::endl;
        continue;
      }
      errorCode = 0;
      // Do the names match? if vendorName not given ignore it (any vendor
      // matches)
      ...
      if (r < 0) {
        ...
        if (!usbvendor.empty()) {
          ...
          warning << "cannot query manufacturer for device: vid=0x" << std::hex
                  << descriptor.idVendor << " pid=0x" << descriptor.idProduct
                  << std::dec << " : ";
        }
      } else {
      ...
      if (r < 0) {
        ...
        if (!usbproduct.empty()) {
          errorCode = LIBUSB_ERROR_IO;
          warning << "cannot query product for device: vid=0x" << std::hex
                  << descriptor.idVendor << " pid=0x" << descriptor.idProduct
                  << std::dec << " : ";
        }
      } else {
...
int USB::transfer(bool receive, const uint8_t function, const uint8_t send[],
  ...
  int nbytes = libusb_control_transfer(*m_device, bmRequestType, function,
                                       value, index, data, dataLen, m_timeout);
